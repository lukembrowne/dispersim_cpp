...
### Plot patterns averaged across species
  ...
  dat_sp %>%
    ...
    summarise_all(mean) %>%
    ggplot(aes(x = mean_cndd, y = sp_shannon, fill = allelic_shannon)) +
...
#### MATRIX PLOTS with text in square
  ...
  dat_sp %>%
    ...
    summarise_all(mean) %>%
    ggplot(aes(x = mean_gndd, y = mean_cndd, fill = sp_shannon)) +
  ...
  dat_sp %>%
    ...
    group_by(mean_cndd, mean_gndd) %>%
    summarise_all(mean) %>%
    ggplot(aes(x = mean_gndd, y = mean_cndd, fill = allelic_shannon)) +
    geom_tile() + theme_bw()  + scale_fill_distiller(palette = "Spectral", 
                                                     limits = c(0, 4)) +
  geom_text(aes(x = mean_gndd, y = mean_cndd,
...
#### BOX PLOTS  
  ...
  dat_sp %>%
    ...
    group_by(sim_id) %>%
    summarise_all(mean) %>%
    ggplot(aes(x = factor(mean_cndd), y = sp_shannon, col = factor(mean_gndd))) + 
  ...
  dat_sp %>%
    ...
    group_by(sim_id) %>%
    summarise_all(mean) %>%
    ggplot(aes(x = factor(mean_cndd), y = allelic_shannon,
               col = factor(mean_gndd))) + 
    geom_boxplot() + theme_bw() 
  ...
  dat_sp %>%
    filter(abundance > 0) %>% # only species with positive abundance 
   # filter(mean_cndd != 1) %>%
   # filter(abundance_rank < 10) %>%
   # mutate(neutral = ifelse(mean_cndd == 1, 1, 0)) %>%
    group_by(sim_id) %>%
    summarise_all(mean) %>%
   # group_by(mean_gndd, neutral) %>%
    group_by(mean_gndd) %>%
   ...
   # group_by(mean_gndd, neutral) %>%
    ...
    summarise_all(mean) %>%
    ggplot() +
    geom_point(aes(x = mean_gndd, y = sgdc), size = I(5), pch = 19) + 
    geom_hline(aes(yintercept = 0)) + theme_bw()
  
  ### Averaging across CNDD
  dat_sp %>%
    filter(abundance > 0) %>% # only species with positive abundance 
  #  filter(mean_gndd != 1) %>%
  ...
  # Scatter plot of allelic vs species shannon
...
### Calculate SGDC and plot based on rank abundance
  ...
  dat_sp %>%
   ...
   # filter(mean_cndd != 1) %>% # Filter out neutral models
    ...
    geom_hline(aes(yintercept = 0)) + facet_grid(. ~ mean_gndd)
...
dat_sp_allsteps %>%
  ...
  summarise_all(mean) %>%
  ggplot(aes(x = step, y = sp_richness, color = factor(seed_disp_dist))) + geom_line()
...
dat_sp_allsteps %>%
  ...
  group_by(seed_disp_dist, step) %>%
  summarise_all(mean) %>%
  ggplot(aes(x = step, y = allelic_richness, color = factor(seed_disp_dist))) + geom_line()
...
landscape_genotype_sim_006_sp_000_FINAL
